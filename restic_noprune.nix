...
in
{
  options.services.restic_noprune.backups = mkOption {
    ...
    type = types.attrsOf (types.submodule ({ config, name, ... }: {
      options = {
        ...
        enablePrune = mkOption {
          ...
          default = true;
          description = lib.mdDoc ''
            If true and pruneOpts are set, prune is run after forget. Otherwise, prune is not run.
            This option is recommended to be false for low memory setups.
          '';
        };
        ...
        enableCheck = mkOption {
          ...
          default = true;
          description = lib.mdDoc ''
            If false, skips the check part of the backup. Combined with enablePrune this will reduce memory consumption
            and enable small systems to run backups.
          '';
        };
  ...
  config = {
    warnings = mapAttrsToList (n: v: "services.restic_noprune.backups.${n}.s3CredentialsFile is deprecated, please use services.restic_noprune.backups.${n}.environmentFile instead.") (filterAttrs (n: v: v.s3CredentialsFile != null) config.services.restic_noprune.backups);
    assertions = mapAttrsToList (n: v: {
      assertion = (v.repository == null) != (v.repositoryFile == null);
      message = "services.restic_noprune.backups.${n}: exactly one of repository or repositoryFile should be set";
    }) config.services.restic_noprune.backups;
    systemd.services =
      mapAttrs'
        (name: backup:
          let
            ...
            pruneCmd = optionals (builtins.length backup.pruneOpts > 0) [
              (resticCmd + " forget " + (if backup.enablePrune then "--prune" else "") + " " + (concatStringsSep " " backup.pruneOpts))
            ] ++ (if backup.enableCheck then [(resticCmd + " check " + (concatStringsSep " " backup.checkOpts))] else []);
