{
  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (system: {
      ...
      packages = {
        mastodonSrc = let pkgs = nixpkgs-unstable.legacyPackages.${system};
        in with pkgs; (callPackage ./mastodon-pkg/source.nix { });

        mastodon = let pkgs = nixpkgs-unstable.legacyPackages.${system};
        in with pkgs;
        ...
        mastodonEmojiImporter =
          let pkgs = nixpkgs-unstable.legacyPackages.${system};
          in with pkgs;
          (callPackage ./mastodon-pkg/mastodonEmojiImporter.nix { });

        mastodonGems = let pkgs = nixpkgs-unstable.legacyPackages.${system};
        in with pkgs;
        (callPackage ./mastodon-pkg/bundlerEnv.nix { inherit self; });

        monero-feather = let pkgs = nixpkgs.legacyPackages.${system};
        in with pkgs; (qt6.callPackage ./monero-feather.nix { });
